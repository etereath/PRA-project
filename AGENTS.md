...
- 任务 13.5 规划中的唯一无人值守写动作是受控 `SYSTEM_EMERGENCY` 紧急下架；当前必须保持禁用。只有在 13.5-6 完成人工 Incident 闭环、策略评审、策略结构迁移、实机验收并由管理员显式启用后，才允许对极端低价商品等待另一个完整且已导入的 `ONLINE_PULSE`、二次读取并确认“紧急情况仍存在且没有复核结果”，再创建明确授权与单一 `SET_OFFLINE` 任务。该动作必须复用 v5、Importer、写锁和唯一 RECONCILE，且不得自动重新上架；不使用每日次数上限或冷却，人工重新上架后仍按正常 S3/S4 流程重新评估。
