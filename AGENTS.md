...
- 默认只运行当前改动的直接测试与一级依赖。预计超过 4 个测试文件、项目测试量的 20% 或 2 分钟时，视为扩大验证范围并先申请授权。单个失败用例默认最多重跑一次；不得用大量重复运行碰绿或证明偶发性。
- 文档、状态页、历史分支归档和单纯推送默认不运行 pytest、Ruff 或系统冒烟。历史基线已有失败只记录，不自动修复；测试数量不是完成度指标。
- 新增测试前必须说明对应生产风险、现有测试为何不足以及能否扩展既有 fixture。禁止为增加数量、重复相同状态转换、纯文本展示或历史恢复快照新增测试体系。
