...
3. 评阅以用户指定对象为准。文档评阅检查语义、规则冲突、引用与编码，不追查无直接关系的代码或 CI 失败；代码复审检查原问题及直接回归，出现新硬安全证据或用户明确要求扩大时除外。
...
| 测试组织、接口适配、交付细则 | [开发执行参考](docs/development_workflow.md)，仅需要时读取相关节 |
| 其他主题 | [文档索引](docs/index.md)中对应条目 |

.archive/ 是人工或网页端参考归档，不作为 Codex 工作规范，默认不检索、不读取；仅用户当前明确点名时使用。历史引用不恢复其规则身份。
...
- 修改接口、DTO、状态或持久字段时，先查生产与测试调用处并同步适配。开发期默认定向验证；授权、事务、并发和恢复保留必要真实集成，不因父任务评级高就重做全系统验收。
...
- 审核默认只评阅，用户明确要求修改时才实施。按用户指定时机推送；文档修改本身不等于推送授权，现有 CI Gate 也不能自行绕过。
- 结果简述改动、验证及相关限制，按本轮实际操作披露提交、推送等状态，不逐项填六个“未操作”。较大实现区分 Implementation Review 与 Stage Goal；文件、测试、CI、实机及部署证据不互相替代。
