...
def test_settlement_handler_uses_existing_run_lease_and_creates_only_provisional(
    ...
    assert completed_event.payload["audit_receipt"]["readback_passed"] is True
    assert completed_event.payload["sales_plan_input"]["plan_input_status"] == (
        "INELIGIBLE"
    )
...
def test_plan_input_handler_registers_no_platform_write_side_effect(
    ...
    runtime.init_schema()
    automation_repository = AutomationRepository(runtime)
    settlement_time = datetime(2026, 8, 1, 12, 0, tzinfo=timezone.utc)
    scheduled_for = settlement_time + timedelta(minutes=5)
    jobs = ensure_default_automation_jobs(
        automation_repository,
        platform_name="platform",
        now=scheduled_for,
    ...
    settlement_run, _ = automation_repository.ensure_run(
        job=settlement_job,
        scheduled_for=settlement_time,
        time_context=time_service.classify(settlement_time),
        initial_status=AutomationRunStatus.SCHEDULED,
        now=settlement_time,
    )
    handlers = build_sales_settlement_handlers(
    ...
    AutomationService(
        ...
        operational_time=time_service,
        clock=lambda: settlement_time + timedelta(minutes=1),
        owner_token="settlement-before-plan-test",
        max_runs_per_cycle=1,
    ).run_cycle()
    assert (
        automation_repository.get_run(settlement_run.run_id).run_status
        is AutomationRunStatus.SUCCESS
    )
    AutomationService(
        automation_repository,
    ...
    completed = automation_repository.get_run(run.run_id)
    assert completed is not None
    assert completed.run_status is AutomationRunStatus.SKIPPED
    assert completed.output_manifest_sha256.startswith("sha256:")
    completed_event = next(
        event
    ...
    assert completed_event.payload["recovered_from_settlement_snapshot"] is True
    assert completed_event.payload["plan_input_status"] == "INELIGIBLE"
