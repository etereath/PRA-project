...
```text
[OK] runtime DB 初始化成功
[OK] schema version exact v15
[OK] v15 任务、观察、Incident、Outbox 和 COMMIT 批次结构完整
...
- runtime DB 初始化成功。
- schema version 必须精确为 `app.runtime_schema.LATEST_RUNTIME_SCHEMA_VERSION`，当前为 v16，迁移记录连续包含 `1..16`。
...
- v15 health check 要求 Outbox、RetryAuthorization、平台身份/库存观测、任务旧价、Incident 事件和 COMMIT 批次账本的列、外键、状态约束、唯一约束及关键索引完整。
- 创建 runtime task。
- `dedupe_key` 去重有效。
...
- `runtime schema health check`：检查当前 v15 必需表、列、约束和索引，避免“伪升级”数据库通过检查。
