...
> 接续更正（2026-07-23 复审）：下文第 2.3 节和第 3 节中的“读取完整
> pending 列表”仅为首次交接时的历史描述，不再是当前运行规范。任务14完成并
> 审查前，production 入口必须由操作人员明确传入一个或多个 `--task-id`；
> `pending` 只表示候选状态，禁止调度器自动扫描全部 pending 并发布 COMMIT。
> 完整冻结边界见接续修复报告第 10 节。本说明采用接续勘误，不改写历史正文。
