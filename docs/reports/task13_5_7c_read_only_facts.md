...
- `/today`：当前 PRA 交易日、OPEN 状态、销售事实状态、销量、成交金额、均价、当前产品
  库存资料、品种/等级表、今日待办和自动化时间轴；
- `/database`：业务数据；
...
- Mobile Review 的有效、无效、过期和已处理只读状态。
...
可信零只在权威事实明确记录为 0 时展示。缺失、不完整、过期和失败不会伪装成 0。当前
...
- 7C 专项：`18 passed`；
- 受影响集成：`210 passed, 3 subtests passed`；
- 完整 pytest：`1170 passed, 3 skipped, 97 subtests passed`；
