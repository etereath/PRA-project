...
本文用于整理任务12当前工作区，并约束后续向 GitHub 和 Google Drive 的同步范围。本文不改变任务状态；任务状态仍等待审查方确认。

## 1. 同步原则

- GitHub 保存可复现的源码、测试、配置样例、维护文档和经过筛选的任务12证据。
- Google Drive 保存供人工审查的最终报告、证据索引和当前状态说明，不复制整个源码仓库。
...
6. GitHub Draft PR 创建后，再把 PR 链接写入 Google Drive 新增交接材料。
7. Google Drive 上传或创建文档后，重新打开核对标题、内容和链接，不以上传成功提示代替验收。
...
- 尚未暂存、提交、推送、创建 PR 或上传 Google Drive。
- 尚未改变任务12状态。
