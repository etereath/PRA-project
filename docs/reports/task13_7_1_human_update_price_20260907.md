...
授权有效期取确认预览后 10 分钟与 Task TTL 的较早值；请求有效期同样不越过授权。初版实现曾要求价格事实有来源、观察时间且在 30 分钟以内；2026-09-08 现场复盘后，该时间门禁被更正为只要求原价格存在，因为 v4 在真实写前会重新读取页面并比对 `expected_old_price`，不一致时零写停止。价格漂移不会触发旧 one-shot 自动写回。已接受的授权重放绑定同一主体、幂等身份、确认摘要和 Task 集合。
