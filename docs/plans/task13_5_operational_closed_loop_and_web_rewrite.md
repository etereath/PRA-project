...
- 业务管理复用既有输入服务，并新增即时人工任务编排；创建 Task 和真实平台执行授权分成
  两个可审计阶段。普通 `PENDING` 不得无人值守执行；Web 只对明确批次提供受控提交执行。
- 冻结数据库真实库存和平台库存：真实库存代表还有多少花可卖，由人工入库和权威销售差额
  更新；平台库存只代表单平台买家可购上限。实现不可变库存流水、取消恢复和库存预警前先
  完成独立 R4 合同和最小 Schema 评审。
- 固定 Automation 方案允许有限启停、频率/时间和库存阈值配置，不开放脚本或任意 Cron。
...
内容已并入 T13.5-7 的 7B—7F 工作包。Issue #20 已于 2026-08-08 同步为当前
13.5-7 路线。
