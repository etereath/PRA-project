...
- [x] S0–S4、重复提醒、S4 禁止条件和紧急下架恢复通过演练。
- [x] 普通写动作仍被明确任务与授权门禁阻止；`SYSTEM_EMERGENCY` 只有一个受控入口。
...
由 Issue #20 冻结，不再作为开放产品问题。以下细节按实施进度继续冻结或维护：

1. 不营业日、临时维护窗口和节假日的维护方式。
2. 非交易时段脉冲是否降频，以及错过运行的最大补跑窗口。
3. 订单页面可稳定读取的历史天数和分页运营范围仍待长期样本；取消量推导方法已在
   13.5-5 以相邻完整快照的多重集合减少冻结。
4. S4 价格口径、v16 极简策略、配置落点、迁移和回滚已经在 13.5-6 完成；最低安全价为
   `base_cost`，紧急比例为 `0.80`，策略审批人为管理员，且不使用次数上限和冷却。
5. 通知动作、响应时限、飞书真实投递、手机复核和固定公网入口门禁已经在 13.5-6 完成
   验收；后续只维护收件范围和运营文案，不另建通知通道。
