...
动作、目标价格/平台库存、基础成本、映射状态、价格观察时间和阻断原因。价格观察时间供运营
判断与审计，不作为人工改价的新鲜度硬门禁。
...
库存。映射不唯一、原价格缺失、平台状态不可用、存在同身份开放任务、真实库存不可用
...
人工改价执行继续由既有 v4 在写前读取页面价格并与 `expected_old_price` 比较；不一致时零写
停止，写入后回读确认。因此创建与授权阶段只要求原价格存在，不要求其观察时间在固定分钟数
以内；这不放宽上下架状态、映射、权限、锁或未决执行门禁。
