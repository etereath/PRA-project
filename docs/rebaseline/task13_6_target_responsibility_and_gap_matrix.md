...
| ShadowBot identity mapping | 既有 Executor/部署配置 | 只负责 UI 定位并校验授权 snapshot；不是 Product/Mapping authority 或 fallback |
...
|---|---|---|
| Product Master | 新 SKU 只建立版本化商品身份；缺少库存事实显示 `NOT_INITIALIZED`，不得自动建立 0 balance | 2B 实施者；additive schema、受控 import、shadow compare 通过后显式切换，Web/Automation/Authorization 同 gate 读 DB snapshot/version |
| Platform Mapping | `account_id` 由负责人/部署配置显式提供，不从 platform/profile/UI 推断；唯一性为同一 platform/account/product identity 与有效期内最多一个 VERIFIED SKU | 2B 实施者；切换证据包括旧新解析逐项对比、identity digest、mapping snapshot、账号绑定与 rollback 演练 |
...
| Observation | immutable evidence 是历史事实，qualified projection 是可失效的当前事实 | 2C 实施者；公共输出覆盖 contract/provider version、scope/pages/end marker、account/product identity、mapping/capability version、时间/freshness 和 source refs/digests |
| Human Decision / execution | 决定、授权、continuation、执行事实分层 | 2D 实施者复用 #47～#50 链；新决定先记录，已跨副作用边界者按最小范围收口，不恢复 create 后自动 submit |

Product/Mapping 回滚只允许把所有消费入口原子恢复到一个已验证 authority；不得长期 Web 读 DB、Authorization/Automation 读工作簿。旧工作簿可作 import/export/history，ShadowBot JSON 可作 Executor 定位配置，二者都不得成为 Runtime 隐式 fallback。
...
| IG-12 | Product/Mapping 从工作簿迁 Runtime DB 采用 additive schema、account-aware identity、shadow compare 和 explicit cutover；所有运行读入口同 gate 切换，无隐式 fallback；Product 新增不自动制造 0 库存事实 |
...
| `a3485af` listing qualification / READ_ONLY 接线 | ADAPT | 13.7-2C；输出事实与质量，不自行升级 global blocker；接 2B mapping snapshot |
