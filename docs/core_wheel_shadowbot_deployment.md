...
预期结果：import、CLI 帮助、数据库初始化和 health 均为退出码 `0`；初始化输出包含从 `1` 连续到 `app.runtime_schema.LATEST_RUNTIME_SCHEMA_VERSION` 的完整版本序列（当前为 `1..18`），health 输出包含 `ok=True`。数据库应位于临时目录，不写入 wheel 或仓库。
