...
- [reports/shadowbot_ui_fault_injection_20260704.md](reports/shadowbot_ui_fault_injection_20260704.md)：登录失效实机结果，以及白屏、网络异常分类和待验收项。
...
2. 在已完成 `test2` 常驻 Worker 的 `READ_ONLY/FILL_PREVIEW/COMMIT` 实机验收和 8 小时 READ_ONLY 观察基础上，补充提交意图后停止及长期运维样本。
3. 继续扩展登录、网络、元素失效、证据上传失败等故障注入。
