# 干净重建 Runtime Schema v18 与真实库存切换

> 文件名保留 `v17` 仅为兼容既有链接；2026-08-15 起正文和脚本均以 v18 为准。
...
python scripts/clean_runtime_cutover.py prepare `
  ...
  --workspace-dir D:\PRA_Runtime\cutover\runtime-v18-clean
```
...
- 完整旧测试库 SQLite 归档；
- 商品和平台映射工作簿原样归档；
- 健康、已导入商品目录/平台映射且库存为 `PRE_CUTOVER` 的 v18 候选库；
...
只有输出同时满足 Schema v18、`DB_AUTHORITY`、外键违规为 0、商品目录/平台映射数据库
快照一致、SKU 集合完全一致、逐 SKU 库存完全一致和库存合计一致，候选库才可进入激活预览。
...
- 准备清单中的旧库逻辑快照；
- `verify` 输出的候选库逻辑快照；
- 固定确认文本 `REPLACE_TEST_RUNTIME_WITH_CLEAN_V18`；
- `--apply`。
