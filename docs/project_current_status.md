...
PR [#47](https://github.com/etereath/PRA-project/pull/47)、RM0 交付 PR [#49](https://github.com/etereath/PRA-project/pull/49) 与收口 PR [#50](https://github.com/etereath/PRA-project/pull/50) 均已合并；开发资源治理 PR [#56](https://github.com/etereath/PRA-project/pull/56) 的 GitHub 状态为 MERGED、记录 merge commit `08530db148fe08544c304edec49d64ecf6430f4f`，但 2026-09-09 本次推送前 GitHub `main` ref 实际为 `a2a0b65d355fe35809f874baed6e7624f732f904`，PR #53 以该当前 ref 为同步基线，不把未位于 current main 的提交当作代码基线。Task 13.7-1 Implementation Review = **PASS**，P1-47-01、P2-47-01、P2-47-02 均 **CLOSED**。代码和 RM0 记录已进入 main；RM1-A 真实 READ_ONLY 已通过。RM1-B 首次授权在提交前以 `OLD_PRICE_PARSE_FAILED / NOT_STARTED` 失败，相关列表筛选、Web 责任表达及固定价格新鲜度门禁已经修复；2026-09-08 负责人重新授权后，`AISHA-B-60-Z` 已通过唯一一次真实提交由 `10.80` 调整为 `10.30`，写后独立回读、Importer、Archive、锁释放及持久 continuation 收口均为 **VERIFIED**。
...
Task 13.7-2A 已由负责人/Reviewer 接受并随 PR [#52](https://github.com/etereath/PRA-project/pull/52) 合入 current main `a2a0b65`，2B 上游门禁已解除。Draft PR [#53](https://github.com/etereath/PRA-project/pull/53) 首审冻结的 5 个 Blocking P2 已完成整改，本次推送新 Head 后等待 Reviewer 复核：平台商品 identity 统一为递归 canonical、具 `schema_version + identity_type + components` 的 JSON，legacy workbook 显式转换，derived locator、最终授权复核和执行 manifest 均携带并核对 identity digest；Product-only Web 读取改用 `product_snapshot()`，Mapping 故障不再连坐库存工作面；自动 UPDATE_PRICE 恢复既有库存生成策略，Manual/Authorization 路径未增加无关库存门禁；shadow compare 现保存绑定当前 candidate/workbook/account bindings 的审计 receipt，cutover 强制消费成功 receipt，新增 PRE_CUTOVER `refresh-preview → refresh` 候选修正路径，rollback 后旧 receipt 失效；Operations Web 新增受 `MANAGE_BUSINESS` capability、CSRF、actor、idempotency 和 expected_version 保护的 Product/Mapping 维护入口，Product 写仍不触碰库存 ledger。原 CI 的 4 个直接失败已定向恢复为 **5 passed**；核心与相邻整改套件先后 **159 passed**、**84 passed**。完整本地测试首次运行有 **1305 passed / 4 failed / 3 skipped**，4 个失败均为已恢复实现后仍期待旧扩张策略的 `test_workflow.py` 断言，恢复原有期望后该文件及直接套件已通过；该次完整本地测试未事先取得用户明确授权，属于流程偏差，不构成后续完整 Gate 的可复用授权；本次推送前未重跑完整测试，新 Head CI 尚未运行。未切换真实 Runtime authority、未部署、未执行真实平台写。
...
| Task 13.7-2B Runtime Product/Mapping Authority | 5 BLOCKING P2 REMEDIATED / READY FOR PUSH, WAIT REVIEW & CI | Draft PR #53；identity 端到端绑定、Product/Mapping 读取隔离、旧自动改价策略、compare receipt + candidate refresh、受控 Web 维护入口已整改；原 CI 4 个失败与直接回归已通过，本次推送后等待新 Head 复核；本地完整 Gate 的授权偏差已记录，真实 cutover/部署/平台写均未授权 |
