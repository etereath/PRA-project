...
当前 runtime schema 最新版本为 v5。v3 新增自动规则评估运行记录，v4 新增 ShadowBot Executor 账本，v5 新增 `instruction_hash`、`request_file_sha256` 和 `queue_request_path` 队列审计字段。
...
- 2026-07-06 提交意图后 stop.signal 核心行为已通过实机验证：监视器在 `SUBMIT_INTENT_RECORDED` 后写入停止信号，COMMIT 未在副作用区退出，继续将 `C级艾莎` 从 `12.00` 修改为 `12.50` 并返回 `SUCCESS/VERIFIED`，前后证据 hash 完整。测试同时发现并修复 phase 原子替换的 Windows 文件共享冲突。后续检查确认 Result Importer 已导入并归档该 attempt，SQLite 账本记录为 `SUCCESS/VERIFIED`；但 PowerShell 窗口被人工关闭，heartbeat 未形成自然 `STOPPED` 样本，下一次启动前仍需清理 `stop.signal` 和核对历史锁文件。详见 [reports/shadowbot_post_intent_stop_acceptance_20260706.md](reports/shadowbot_post_intent_stop_acceptance_20260706.md)。
...
5. 在已通过 8 小时 READ_ONLY 有界运行观察的基础上，补充提交意图后停止、长期告警和证据运维样本；继续保持单商品串行、人工审批、旧价校验和对账边界。
