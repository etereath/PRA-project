...
> 历史分支发布说明（2026-09-09）：本页随
> `codex/task13-5-7f-runtime-master-data` 的本地恢复快照一并保存，记录的是
> Task 13.5-7F 当时的候选实现与验收状态，仅供后续审查和选择性复用。Task 13.5
> 已在当前主线停止并由后续路线取代；本分支不得作为当前生产状态、部署版本或真实平台
> 操作授权。当前项目状态以 GitHub 最新 `main` 中的同名文档为准。
...
手机紧急复核均不再读取商品或映射 XLSX；两份工作簿只允许在干净候选库准备时一次性导入。
2026-08-16 已完成 canonical 真实库 v18 切换和库存 bootstrap：Schema 健康、外键违规为 0、
库存权威为 `DB_AUTHORITY`，12 个 SKU 库存合计 29。旧 v14 已完整归档；后续回滚或再次重建
仍需新的维护窗口授权。

2026-08-30 运营 Web 已补齐“业务管理 → 任务队列”只读页面：同一页面显示待发送、排队中、
执行中、等待结果回收和失败/人工确认任务，并同时展示任务传递、影刀执行端和结果回收状态。
页面复用现有 Runtime Task、执行账本和文件队列，不新增第二套控制面；支持人工、自动、紧急
保护来源及执行阶段筛选，完成历史仍进入数据库。真实库只读检查时显示 2 项待发送和 1 项需要
处理，未投递或重试任务。

同日任务队列增加人工批量取消：只允许取消尚未进入 `inbox/working/results` 的待发送或执行
失败任务，确认后在一个事务内写入 `CANCELLED` 和任务状态历史。运行中、等待结果回收、待复核、
状态已变化或队列无法完整核对的任务均整批拒绝；页面不会删除任务记录。真实库验收只检查选择
按钮和确认弹窗，未实际取消当前任务。
...
2026-08-29 已补齐正式商品状态定时 Handler：`FULL_MARKET_SCAN` 在同一计划窗口派生
`LISTING_STATUS_SCAN` 与 `ORDER_SCAN`，商品子链原样复用任务 13 v5 双页扫描、结果导入、
不可变 Product Observation 和归档。显式只读扫描模式仅限制本进程注册的 Handler，不会
物化或执行日结、规则生成和复核维护。真实 `16:10` 窗口已完成父 Run、两个子 Run、两个
Importer 与 Archive，三者均为 `SUCCESS`；商品和订单批次均为 `ACCEPTED`，范围与尾部标记
完整，平台操作和业务任务新增均为 0。

同日已补齐真正的 `ONLINE_PULSE`：继续复用任务 13 v5 请求、Worker、列表控制流和归档，
但请求冻结为 `scan_scope=online`，执行端不会选择或读取“待上架”页。单页事实直接进入 v14
已有的 `product_observation_batches/items`，不会伪造只允许双页完整的
`listing_sync_snapshots`，也不会用上架中缺席推断下架。真实 `17:40` 计划窗口 Run 为
`SUCCESS`，批次为 `ACCEPTED`，范围完整且尾部已确认；3 项观察全部为上架中正向事实，
`waiting_scan_complete=false`，执行 timing 不包含待上架扫描，平台操作与业务任务均无新增。
请求和结果已归档且校验和一致，长期 Worker 保持 `RUNNING`。

2026-08-30 又在 canonical v18 上完成常驻只读无人值守周期：09:50、10:00、10:20 三次
自然 `ONLINE_PULSE` 均为 `SUCCESS / ACCEPTED`；10:10 的 `FULL_MARKET_SCAN` 父 Run、
`LISTING_STATUS_SCAN` 与 `ORDER_SCAN` 两个子 Run 均为 `SUCCESS`，商品和订单批次均完整并
确认尾部。同分钟快速扫描按覆盖规则合并，没有重复操作小程序；归档校验和一致，新增平台
操作和业务任务均为 0。旧 Queue Service 在验收准备时已因一次 Windows 文件替换
`PermissionError` 退出，恢复后的同库实例跨过 10:20 扫描并持续健康；交付时 Automation、
Queue Service 与 Worker 均保持新鲜 `RUNNING`。详细证据见 7F 实施与验收报告第 16 节。

同日管理员又通过新运营 Web 完成一次真实人工下架验收：一个范围操作生成艾莎 B/C/D 三个
Runtime Task，创建阶段不投递 Queue；独立执行授权把三个明确 Task 绑定为一个 v5
`set_offline` 批次。批次 3 项全部经后置回读验证，未知、部分完成和失败均为 0；请求、结果、
Importer ACK 和 Archive 校验一致，活动队列归零。随后 10:40 自然 `ONLINE_PULSE` 继续成功，
与下架结果一致但不以单页缺席替代下架回读。该验收不构成后续批次的持续写授权。
...
本轮同时确认数据库库存是农场真实剩余库存，平台库存只代表特定平台买家可购上限；两者
不要求数值相等，平台目标库存可以高于真实库存，也不能把各平台目标库存相加后当作超售。
数据库余额没有新流水时自动跨 PRA 交易日延续，不需要人工确认转结。`20` 扎按品种作为
真实剩余库存的运营安全余量；现已由库存流水回调汇总同品种各等级余额，复用既有
Incident/Outbox 完成触发、抑制、重复提醒和恢复，不作为平台初始额度
上限。数据库库存由人工有符号调整和权威销售事实的幂等差额自动更新，并支持取消恢复、
不可变流水和品种安全余量预警；该能力已在 7D 的 R4 合同和最小 Schema 评审后落地。创建 Task
...
2026-08-30 的 7F 验收整改把运营侧任务创建收成一条连续链：范围窗口只选择品种、等级、
平台和动作，随后自动弹出逐项预览；每个具体平台商品分别带入最近记录的价格和平台库存并
允许单独修改。运营者只确认一次，后台仍按“创建 Runtime Task → `prepare_execution` →
`submit_execution`”三个既有服务边界执行，不扫描其他 `PENDING`，也不复制 Queue/Worker
发布逻辑。创建后发送失败的 Task 会保留在“当前任务”供既有授权入口继续处理。
人工改价、加/降价、上架和下架任务遇到最近定时扫描不完整、质量不足，或价格、平台状态记录
过期时，在最终确认弹窗逐项提示，不再直接阻断创建或发送；人工上下架预发布也不以旧扫描
阻断或直接判定已完成，实际状态由执行端页面预检。真正缺少平台状态、映射/价格/目标值错误、
未解决复核、写锁和开放任务冲突仍阻断。AUTOMATION 来源的上架任务继续保留扫描质量硬门禁。
...
2026-08-16 又完成一次测试责任去重。旧 Web 的 `test_web.py` 与
`test_web_security.py` 已在 7F 删除，本轮没有继续删除跨层安全测试；仅把同一责任内重复的
配置脱敏、会话跳转、一级路由方法、静态资源、系统当前状态和运营标签断言归并到既有代表性
测试。所有原断言保留，CSRF、权限、Schema 健康、幂等冲突、事务回滚、Queue/Worker/
Importer 与真实写授权门禁均未删除。pytest 收集数由 1243 降至 1237；Web 专项为
`59 passed`，完整回归为 `1234 passed, 3 skipped, 82 subtests passed`，耗时 439.57 秒；
隔离系统冒烟为 `16 passed, 0 failed`。该整理减少 6 个重复测试项，但没有显著缩短全量耗时，
因此不把它描述为性能优化。
...
- 创建任务：按品种、等级、平台多选，逐项设置价格/平台库存；逐项预览不提交任务，点击
  “继续确认”后才打开小型最终确认弹窗。运营者只在该窗口确认一次，后台仍分开审计 Task
  创建和真实平台执行授权。四类人工动作的扫描质量或价格/状态新鲜度不足只在最终确认中提示；
  人工上下架随后由执行端实时核对，自动任务仍按门禁等待合格扫描。
