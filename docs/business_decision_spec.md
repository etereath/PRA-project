...
截至当前代码状态，项目已经不再只是“Excel 规则原型”，而是进入了：

`Excel 业务输入 + SQLite 运行态任务系统 + 运行态运营闭环增强第一版`
...
- Web 管理页、表格管理页、执行回写页
- SQLite 运行态数据库骨架
- 运行态任务、复核任务、通知记录、状态历史表
- 运行态 CLI 第一阶段闭环
- 运行态 Web 登录、复核处理和状态历史查看 MVP
- notification_logs 接入 review 主流程的 MVP
- 旧 Excel 人工介入链路收口到只读兼容
- Web 运行态高频筛选与通知详情增强
- `NotificationSender / MockNotificationSender` 抽象
- `expire-review-tasks` CLI 与超时保守策略

当前还没有完成的部分包括：

- 真实通知渠道接入
- 旧 Excel 人工介入入口进一步弱化或隐藏主导航
- 真实平台与真实 RPA 执行
- 手机端 review 页面、review token 闭环与完整权限系统

因此，本文档中凡是涉及“SQLite 运行态持久化”的内容，应理解为：
...
9. 预测性销售相关的基础模型与服务边界
10. SQLite 运行态数据库骨架
11. 运行态任务、人工复核、通知记录、状态历史的基础持久化能力
12. 运行态 CLI 第一阶段闭环
13. 运行态 Web 登录、复核处理和状态历史查看 MVP

### 3.2 当前实现的真实角色

当前系统更准确的角色是：

`业务规则验证器 + 预测性销售决策核心 + 任务生成器 + 运行态任务系统骨架`
...
- 风险策略切换边界
- 运营提醒类任务与人工复核任务基础生成
- SQLite 运行态任务、复核、通知、状态历史表结构
- 运行态人工复核闭环 MVP（Session 登录、复核处理、状态历史查看、防重复提交）
- notification_logs 跟随 review_task 自动生成的通知记录层
...
当前代码已经引入 `trade_date` / `trade_open_at` / `clearance_start_at` / `trade_close_at` 的基础交易窗口模型与服务边界。

后续这一节的重点不再是“补正式模型”，而是验证以下业务场景中的一致性：

- 任务生成时对 `trade_date` 的归属是否一致
- 清库存阶段是否稳定按 `clearance_start_at` 生效
- `17:00` 后是否稳定限制新的上架和改价任务
- 收尾类任务、人工复核和状态同步任务是否正确穿过收盘后阶段
...
- 数据库库存是农场真实剩余库存；没有新库存流水时自动延续到下一 PRA 交易日，不重置，
  也不要求人工确认转结；
- 平台库存是平台买家的可购上限，不是销量、真实库存占用或预留量。单个平台目标库存可以
  高于真实库存，各平台目标库存也不得相加后与真实库存比较；
- `20` 扎按品种共享，表示销售消耗接近真实库存边界时的运营安全余量。它依据同品种各等级
  的数据库真实剩余库存和已实现销售判断，不从平台目标库存中扣除，也不用于按等级比例
  分配平台额度；
- 初始平台额度允许依据经验和销售预测显著高于真实库存，后续再依据定时扫描得到的实际
  销售进度调整。具体自动调整策略未冻结前，`20` 扎只作为预警/人工处置边界，不自动创建
  平台写任务；
...
1. `trade_date` 已具备基础模型与服务边界，但仍需在任务生成、阶段判断和收盘后限制等业务场景中继续验证一致性
2. 当前 Product 仍偏 SKU 视角，Product 与 `forecast_group_key` 的映射仍需固化
3. `recommended_price` 当前仍处于从 Product 兼容字段向 PriceForecast 独立对象迁移的过程中；迁移期间允许 Product.recommended_price 作为兼容字段存在，但新的业务逻辑应优先读取 PriceForecast.recommended_price；若没有 PriceForecast，则可回退到 Product.recommended_price。
4. 包装能力、冷库压力、短缺风险已经具备基础模型或运行态骨架，但仍需与真实运营输入打通
5. 运营提醒类任务与人工复核类任务已进入运行态骨架，且运行态人工复核闭环 MVP 已实现，但旧 Excel 人工介入链路仍需收口
6. 当前价格决策尚未完整支持交易阶段、单次最大降价幅度、双最低价体系
7. 当前库存逻辑尚未完整区分 `actual_stock_qty`、`reserved_qty`、`safety_buffer_qty`、`field_buffer_qty`
8. `forecast_group_key = variety + grade` 已进入模型与服务边界，但映射约束和运营输入口径仍需进一步固定
9. 当前仍未正式支持等级兼容调拨建模
10. 全场共享冷库容量状态对象已具备基础模型与运行态骨架，但业务闭环仍需补齐
...
1. 旧 Excel 人工介入链路收口到只读兼容
2. Web 运行态高频筛选与通知详情增强
3. `NotificationSender / MockNotificationSender` 抽象
4. `expire-review-tasks` CLI
5. 超时保守策略与 `fallback_to_safe_default` 记录
...
- 已完成单平台、单窗口、单 Worker 严格串行多商品的真实 ShadowBot 改价闭环，但暂不承诺无人值守生产改价
- 已接入蚂蚁花团供应商真实 RPA；暂不扩展第二平台、跨平台混合批次或多 Worker 并发
- 暂不做完整权限系统
- 暂不迁移 Excel 主数据
- 暂不直接做手机端页面
